...
const ShortcutCustomizeUI = {
  ...
  async build(options) {
    ...
    for (let command of commands) {
      ...
      if (this.available) {
        const resetButton = keyCombination.appendChild(this.createOperationButton('input',
                                                                                  () => reset()));
        resetButton.type = 'image';
        resetButton.src = 'restart.svg';
        resetButton.style.marginLeft = '8px';
        resetButton.setAttribute('title', this.getLocalizedKey('Reset') || 'Reset');
        //const clearButton = keyCombination.appendChild(this.createOperationButton('button', () => clear()));
  ...
  createOperationButton(type, onCommand) {
  ...
  keyNameMapLocales: {
    ...
    fr: {
      ...
      Home: ['Début'], 
      End: ['Fin'], 
      PageUp: ['PageHaut', 'Page précédente', 'PgPréc'], 
      PageDown: ['PageBas', 'Page suivante', 'PgSuiv'], 
      Delete: ['Suppr'], 
      MediaNextTrack: ['Piste suivante'],
      MediaPrevTrack: ['Piste précédente'],
