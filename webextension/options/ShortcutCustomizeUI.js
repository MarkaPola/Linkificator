    ...
    for (let command of commands) {
      ...
      const reset = () => {
        ...
        browser.commands.getAll().then(aCommands => {
          for (let defaultCommand of aCommands) {
            ...
            list.dispatchEvent(createEvent(command.name, 'default'));
            item.classList.remove('error');
            apply();
            break;
          }
      ...
      if (options.showDescriptions) {
        const name = `${command.description || command.name}: `
              .replace(/__MSG_(.+?)__/g, (aMatched, subMatch1) => browser.i18n.getMessage(subMatch1));
      ...
      if (this.available) {
        const resetButton = keyCombination.appendChild(document.createElement('input'));
