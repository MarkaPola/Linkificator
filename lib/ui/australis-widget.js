...
const WidgetTrait = function (options) {
    ...
    this.woptions = {
        onCreated: (function (node) {
            ...
            if (this.handlers.mouseover) {
                ...
                unload(function () {node.removeEventListener('mouseover', this.handlers.mouseover);}, node);
            }
        }).bind(this),

        onClick: (function (event) {
            if (this.handlers.click) {
                this.handlers.click(event, this.widget);
            ...
            if (event.button == 1 || (event.button == 0 && event.altKey == true)) {
		        if (this.handlers.middleclick) {
                    this.handlers.middleclick(event, this.widget);
		            event.stopPropagation();
		            event.preventDefault();
	            }
            }
	        ...
	        if (event.button == 2 || (event.button == 0 && event.shiftKey == true)) {
		        if (this.handlers.rightclick) {
                    this.handlers.rightclick(event, this.widget);
		            event.stopPropagation();
		            event.preventDefault();
	            }
            }
    ...
    unload(function () { this.destroyWidget(); });
