...
function Configurator () {
    ...
    var defaults = {
        ...
        support: {email: true, about: false, standard: true},

		customRules: {support: {before: false, after: false},
					  rules: {beforeList: [], afterList: []}}
    };
	...
	currents.raw = {
		support: {
			...
			get standard () { return currents.support.standard; },
			set standard (data) { currents.support.standard = data; }
		},

		customRules: {
			get rules () { return JSON.stringify(currents.customRules.rules); },
			set rules (data) { currents.customRules.rules = JSON.parse(data); }
		},
    ...
    return {
        ...
        linkifyURL: function (URL) {
			let list = currents.domainList;
            let flag = list.type == 'white';

			let index = 0;
			while (index != list.domains.length) {
				if (currents.domainList.regexp) {
					if (URL.match(RegExp(list.domains[index++], "i"))) {
						return flag;
					}
				} else {
					if (URL.toLowerCase().indexOf(list.domains[index++]) != -1) {
						return flag;
					}
				}
			}
