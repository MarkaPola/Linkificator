...
exports.main = function (options) {
	...
	//require('sdk/window/utils').getMostRecentBrowserWindow().BrowserOpenAddonsMgr("addons://detail/" + encodeURIComponent('linkificator@markapola') + "/preferences");
	...
	pageMod.PageMod({
        ...
        onAttach: function (worker) {
            ...
            worker.port.on('to-page', function () {
				...
				if (controler.isActive() && controler.isValidDocument(tab) && configurator.linkifyURL(tab.url)) {
					process('parse', tab);
				}
			});
	...
	controler.on('statistics', function () {
		...
		if (controler.isValidDocument(tab) && configurator.linkifyURL(tab.url)) {
