...
exports.main = function (options) {
    ...
    tabs.on('activate', function (tab) {
        ...
        if (tab.readyState && (tab.readyState == 'loading' || tab.readyState == 'uninitialized')) {
        ...
        onAttach: function (worker) {
            ...
            worker.port.on('document-changed', function () {
                if (workers.indexOf(worker) !== -1) {
                    controler.setStatus(tab);
                    worker.port.emit(controler.linkifyURL(tab) ? 're-parse' : 'undo', configurator.properties);
                }
            });
...
if (require('./util/system').versionCompare(require('sdk/system').version, "30.0") == -1) {
