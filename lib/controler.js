...
function Controler (configurator) {
		...
		this._panel = {
			on: function (event, action) {
				...
				if (event === 'options') {
					...
					self._widget.panel.port.on('filter', function () {
						execute(action);
					});
				}
			}
	...
	Widget.prototype = {
		...
		setStatus: function (tab) {
			...
			if (isActive() && isValidDocument(tab)) {
				if (filteredURLs.isFiltered(tab.url) {
	...
	function filter () {
		...
		if (isActive() && isValidDocument(tab) && configurator.linkifyURL(tab.url)) {
			let url = filteredURLs.toggle(tab.url);

			if (url) {
				actions.undo && actions.undo(tab);
			} else {
				actions.reparse && actions.reparse(tab);
			}
		}
	}
    ...
    return {
		...
		linkifyURL: function (url) {
			return !filteredURLs.isFiltered(url) && configurator.linkifyURL(url);
		},
