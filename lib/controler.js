...
function Controler (configurator) {
    ...
    function Menu (options) {
        var _options = {localization: options.localization,
                        actions: options.actions};
        delete options.localization;
        delete options.actions;
        
        var _panel = Panel(options);
        if (_options.localization) {
            _panel.port.emit('initialize', _options.localization);
        }
        if (_options.actions) {
            for (let event in _options.actions) {
                let action = _options.actions[event];
	...
	function Widget () {
        ...
        this._menu = Menu({
            ...
            localization: {options: _("panel.options"),
		...
		this._widget.port.on("middleclick", (function () {
				if (this._actions.middleclick) {
					this._actions.middleclick();
				}
			}).bind(this));
		this._widget.port.on("rightclick", (function () {
			if (this._actions.rightclick) {
				this._actions.rightclick();
			}
			}).bind(this));
		...
		this._widget.port.emit(initial.icon);
		...
		this._update = function (tab, status) {
			...
			// set icon
			view.port.emit('icon', status.icon);
			// set tooltip
	...
	Widget.prototype = {
		on: function (event, action) {
			if (event === 'mouseover') {
				...
				this._actions.middleclick = action;
			} else if (event === 'rightclick') {
				this._actions.rightclick = action;
			}
		},
